(function(global) {
  ...
  function parseAlias(id) { // //a  b
    var alias = data.alias; // 配置  {a:"demo/a", b:"demo/b"}
  ...
  function normalize(path) {
    ...
    return (lastC === '/' || path.substring(last - 2) === '.js') ?
      path : `${path}.js`;
  }
  ...
  // 生成决定路径， a => 资源的位置 b =>
  vem.resolve = function(child, parent) {
    if(!child) return '';
  ...
  vem.request = function(url, callback) {
  ...
  Module.prototype.resolve = function() {
    ...
    for (let i = 0; i < ids.length; i++) {
        uris[i] = vem.resolve(ids[i], mod.uri)
    }
    return uris;
  }
  ...
  vem.use = function(list, callback) {
    // 检测有没有预先加载的模块
